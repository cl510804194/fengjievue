	...
	function Dateinput(input, conf)  { 
		...
		if (!root.length) {
			...
			root = $('<div><div><a/><div/><a/></div><div><div/><div/></div></div>')
				.hide().css({position: 'absolute',zIndex:6000}).attr("id", css.root);			
...
//{{{ onShow
		...
		function onShow(ev) {
			...
			$(document).on("click.d", function(e) {					
				...
				if (el.tagName && !$(el).parents("#" + css.root).length && el != input[0] && (!trigger || el != trigger[0])) {
					self.hide(e);
				}
