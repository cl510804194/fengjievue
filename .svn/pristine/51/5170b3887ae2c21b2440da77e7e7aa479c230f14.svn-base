 ...
 */
package com.yjf.esupplier.web.security.realm;
...
public class ShiroWebServiceRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		userLoginOrder.setUserName(token.getLoginData().getUsername());
		userLoginOrder.setPassword(token.getLoginData().getPassword());
		userLoginOrder.setIpAddress(token.getLoginData().getIpAddress());
		userLoginOrder.setValidatePassword(token.getLoginData().isValidatePassword());
		userLoginOrder.setLoginFromTypeEnum(token.getLoginData().getLoginFromTypeEnum());
		userLoginOrder.setUserAgent(token.getLoginData().getUserAgent());
		userLoginOrder.setSecurityCode(token.getLoginData().getSecurityCode());
		userLoginOrder.setPhone(token.getLoginData().getPhone());
		userLoginOrder.setLoginMethodEnum(token.getLoginData().getLoginMethodEnum());
		userLoginOrder.setUserBizTypeEnum(token.getLoginData().getUserBizTypeEnum());
		// 验证码 读session
		LoginResult result = null;
		if (token.isValidate()) {
			result = loginService.validateLoginInfo(userLoginOrder);
		}else {
		...
		if (result.isSuccess()) {
			if(result.getUserInfo().getUserName() == null){
				result.getUserInfo().setUserName("");
			}
